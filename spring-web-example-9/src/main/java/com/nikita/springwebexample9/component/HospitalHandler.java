...
import com.nikita.springwebexample9.document.CombinedResults;
...
public class HospitalHandler {
    ...
    public Mono<ServerResponse> getDoctor(ServerRequest request) {
        int id = Integer.parseInt(request.pathVariable("id"));
        return doctorRepository.findById(id).flatMap(doctor -> ServerResponse.ok()
                .contentType(MediaType.APPLICATION_JSON)
    ...
    public Mono<ServerResponse> AlldetailsbyId(ServerRequest request) {
        ...
        Flux<Object> combinedResults = Flux.merge(result,result1);
